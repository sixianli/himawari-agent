...
const out = `${root}/qualifications/2026-09-12-three-fixes-provider-matrix`;
const runId = "run:a7f2c922-14bb-4516-9525-feaa6ad4354e";
const modelId = "deepseek/deepseek-v4-flash-0731";
const hash = (value) => createHash("sha256").update(value).digest("hex");
// Fixed providers and fixed captured synthetic cases. Every call is independent;
// failed calls retain their full estimated cost and never retry inside a case.
const providers = [
...
async function main() {
  ...
  const unpriced = sources.flatMap((source) =>
    providerMatrixInputs(source.original).map((input) => ({ ...input, sourceRunId: source.id })),
  );
          ...
          const response = await boundedJson(
            await fetch("https://openrouter.ai/api/v1/chat/completions", {
              ...
              headers: {
                ...
                "X-OpenRouter-Title": "Himawari bounded endpoint qualification",
              },
  ...
  console.log("Provider matrix written: " + out + "/summary.json");
}
