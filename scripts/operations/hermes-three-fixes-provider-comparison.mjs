...
const out = `${root}/qualifications/2026-09-12-three-fixes-provider-comparison`;
const runId = "run:a7f2c922-14bb-4516-9525-feaa6ad4354e";
const modelId = "deepseek/deepseek-v4-flash-0731";
const hash = (value) => createHash("sha256").update(value).digest("hex");
// Compare one input dimension at a time; persisted history stays untouched.
...
async function main() {
  ...
  const inputs = sources.flatMap((source) =>
    providerComparisonInputs(source.original).map((input) => ({
      ...input,
  ...
  assert(
    productCostMicros + priorComparisonCostMicros + maximumCostMicros + searchReserveMicros <=
      2000000,
        ...
        const response = await boundedJson(
          await fetch("https://openrouter.ai/api/v1/chat/completions", {
            ...
            headers: {
              ...
              "X-OpenRouter-Title": "Himawari bounded provider comparison",
            },
  ...
  console.log("Provider comparison written: " + out + "/summary.json");
}
