...
INPUTS={'hermes-harness-observer.py': '9bde7c5f3b5c194b9392925d7d43b620614d72d027c29a7e4e181b9a6c814e12', 'hermes-harness-seal.mjs': '7492444061d803e2bee2fafa8081789db48fe72b2cb7ed7a77d4116036ce6afd', 'hermes-harness-start.mjs': 'a56fe8ebf309874f839811b933161ef5495dc15552d8df586ef45878df7df46a', 'probe-protected-runtime.mjs': '8d3c6cc8270dc79d14c1fb97988f1f0dcba97572b623ca49e6cbe1233b92d4de', 'hermes-harness-web-probe.mjs': 'e50661652246f65095602c61e4d215c3dbcd6056f6c49ef61d83ec12585a9751'}
...
def run_user(name,phase,args,env=None,signer_view=False):
 ...
 cmd=['systemd-run','--quiet','--wait','--pipe','--collect','--unit=himawari-harness-qualify-'+phase]
...
def main():
 ...
 assert json.loads((R/'qualifications/2026-09-12-three-fixes-cutover-attempt-v2/result.json').read_text())['deploymentComplete']
 assert json.loads(AUTH.read_text())['runtimeDigest']=='a83239f95583c9c82264059c40e9f59bfddc5355017caedb62affedf4b441a56','CURRENT_INSTALLATION_CHANGED'
 assert sha(B/'preparation.json')=='d85f4b6de731e9801f05a90b5285d5ddbd23e1268e00cdb9086230387c151aa6','PREPARATION_CHANGED'
 prepared=json.loads((B/'preparation.json').read_text())
 assert prepared['buildPassed'] and prepared['sourceArchiveSha256']=='4bdf5a25dc6139be9e5068e1897c626534161430c25cf3849e292ed012e404a4'
 ...
 assert sha(B/'himawari-harness-source.json')=='4ebb7c0bbcb33ca7db68d1d5c00c4e46100ee72383c7367dbfb9b5ff349fd683','SOURCE_MANIFEST_CHANGED'
 manifest=json.loads((B/'himawari-harness-source.json').read_text())
 assert sha(B/'himawari-harness-source.tar.gz')==manifest['archiveSha256'],'SOURCE_ARCHIVE_CHANGED'
 assert manifest['archiveSha256']=='4bdf5a25dc6139be9e5068e1897c626534161430c25cf3849e292ed012e404a4'
 ...
 try:
  ...
  shutil.copyfile(B/'himawari-harness-source.json',Q/'source-manifest.json');(Q/'source-manifest.json').chmod(0o644)
  ...
  signer=run_user('andy','signer-preflight',[RUNTIME/'pi-tools/bin/node',Q/'hermes-harness-seal.mjs','--preflight'],signer_view=True)
  ...
  result=run_user('andy','seal',[RUNTIME/'pi-tools/bin/node',Q/'hermes-harness-seal.mjs'],signer_view=True)
  ...
  reader=runpy.run_path(str(Q/'hermes-harness-observer.py'))
  ...
  subprocess.run(['systemd-run','--unit=himawari-harness-live-observer','--property=RuntimeMaxSec=3700','/usr/bin/python3','-I',Q/'hermes-harness-observer.py'],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
