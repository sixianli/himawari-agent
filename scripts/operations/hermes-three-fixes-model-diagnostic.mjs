...
const runs = [
  "run:ebc44f05-0148-4eb5-915b-60773ea7e0a5",
  "run:a7f2c922-14bb-4516-9525-feaa6ad4354e",
];
const sha = (s) => createHash("sha256").update(s).digest("hex");
const markers = [
  "acceptance-morning.txt",
  "晨光计划验收",
  "执行结果尚未确认",
  "Available tools:",
  "不重复调用",
...
export function summarize(value) {
  ...
  return {
    ...
    sha256: sha(text),
    textSha256: sha(contentText(value)),
    markers: markers.filter((s) => text.includes(s)),
    dates: [...new Set(text.match(/2026-09-\d{2}(?:T[\d:.]+Z)?/g) || [])].slice(0, 20),
  };
}
export function compareToolMessage(message, results, at) {
  const candidates = results.filter((r) => r.toolCallId === message.toolCallId);
  return {
    at,
    toolCallId: message.toolCallId,
    currentRunResults: candidates.length,
    matchesRecordedContent: candidates.some((r) => r.content.textSha256 === message.textSha256),
    providerContent: {
      bytes: message.bytes,
...
export function facts(value) {
  ...
  const walk = (v, path = "", depth = 0) => {
    ...
    if (
      /(?:^|\.)(?:phase|status|outcome|kind|operation|errorCode|reasonCode|cleanup|supervision|stopReason|locale|answerLocale)$/.test(
        path,
      ) &&
...
async function main() {
    ...
    for (const runId of runs) {
      ...
      for (const a of db
        ...
        item.providers.push({
          ...
          model: request.model,
          lastUserMessageIndex: Array.isArray(messages)
            ? messages.findLastIndex((m) => m.role === "user")
            : null,
          duplicateToolCallIds: Array.isArray(messages)
            ? [
                ...messages
                  .flatMap((m) => m.tool_calls ?? [])
                  .reduce(
                    (counts, call) => counts.set(call.id, (counts.get(call.id) ?? 0) + 1),
                    new Map(),
                  ),
            ...
            ? messages.map((m, index) => ({
                ...
                toolCalls: Array.isArray(m.tool_calls)
                  ? m.tool_calls.map((x) => ({
                      ...
                      arguments: summarize(x.function?.arguments),
                    }))
                  : [],
                toolCallId: m.tool_call_id ?? null,
                keys: Object.keys(m),
                reasoning: summarize(
                  m.reasoning_content ?? m.reasoning ?? m.reasoning_details ?? null,
                ),
      ...
      for (const a of db
        .prepare(
          "SELECT payload_ref,created_at FROM run_payload_artifacts WHERE owner_id=? AND agent_id=? AND run_id=? AND purpose='trace' AND operation_key LIKE '%:tool_result:%' ORDER BY created_at",
        )
        .all(...scope, runId)) {
        const result = await payload(a.payload_ref);
        assert.equal(typeof result.toolCallId, "string", "TOOL_RESULT_SHAPE_MISMATCH");
        assert(Array.isArray(result.result?.content), "TOOL_RESULT_CONTENT_MISSING");
        item.toolResults.push({
          at: a.created_at,
          toolCallId: result.toolCallId,
          toolName: result.toolName,
          isError: result.isError,
          content: summarize(result.result.content),
        });
      }
      item.toolMessageComparisons = item.providers.flatMap((provider) =>
        provider.messages
          .filter((m) => m.role === "tool")
          .map((m) => compareToolMessage(m, item.toolResults, provider.at)),
      );
  ...
  const out = `${root}/qualifications/2026-09-12-three-fixes-model-diagnostic`;
