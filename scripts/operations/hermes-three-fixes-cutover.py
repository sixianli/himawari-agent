...
ARCHIVE=R/'releases/2026-09-12-before-three-fixes'
...
BACKUP_ID='before-three-fixes-2026-09-12'
ATTESTATION=STATE/'deployment/startup-attestation.json'
OLD_START=R/'qualifications/2026-09-12-ui-session-installation/hermes-ui-session-start.mjs'
EXPECTED_RECEIPT=sys.argv[3] if len(sys.argv)==4 and sys.argv[1:3]==['--apply','--receipt'] else ''
EXPECTED_CURRENT_RUNTIME='45dd3fa8dbb27b796e51865cfed7ceb871589f42a3b6ce2ca59c5bf362510d7f'
INPUTS={'hermes-protected-registered.mjs': '7904085d39a50e3d2bbab9547ed2e230cc96a9d2592556d8bb5db722b8f6aaa8'}
...
def invoke(phase,args,candidate_view=False,protected=True):
 ...
 cmd=['systemd-run','--quiet','--wait','--pipe','--collect','--unit=himawari-three-fixes-cutover-'+phase]
...
def main():
 ...
 assert sha(Q/'hermes-three-fixes-start.mjs')=='363fcfdc19699b2c51b2b08986bd8ba1313c813a9310df69506d55e5bd7d2808'
 ...
 try:
  ...
  startup=invoke('prepare',[node,Q/'hermes-three-fixes-start.mjs','--prepare',prepared],True);assert startup['preparing'];assert startup['runtimeDigest']==qualified['runtimeDigest']
  ...
  authority_next=AUTH.with_name('runtime.three-fixes-next.json');shutil.copyfile(CANDIDATE_AUTH,authority_next);authority_next.chmod(0o644);os.replace(authority_next,AUTH)
  os.replace(prepared,CONFIG)
  next_unit=UNIT.with_name('himawari.service.three-fixes-next');next_unit.write_text(original_unit.replace(old_exec,'ExecStart='+str(node)+' '+str(Q/'hermes-three-fixes-start.mjs')));next_unit.chmod(0o644);os.replace(next_unit,UNIT)
  ...
  status('starting_three_fixes_service');new_started=True;command(['systemctl','start','himawari.service'])
