...
B=R/'builds/2026-09-13-loop-finalization'
Q=R/'qualifications/2026-09-13-loop-finalization-installation'
W=R/'qualifications/2026-09-13-loop-finalization-cutover'
CURRENT=R/'releases/2026-09-11-control-center'
CANDIDATE=R/'releases/2026-09-13-loop-finalization'
ARCHIVE=R/'releases/2026-09-13-before-loop-finalization'
...
CANDIDATE_AUTH=pathlib.Path('/etc/himawari/loop-finalization-candidate.json')
BACKUP_ID='before-loop-finalization-2026-09-13'
ATTESTATION=STATE/'deployment/startup-attestation.json'
OLD_START=R/'qualifications/2026-09-12-harness-lifecycle-installation/hermes-harness-start.mjs'
EXPECTED_RECEIPT=sys.argv[3] if len(sys.argv)==4 and sys.argv[1:3]==['--apply','--receipt'] else ''
EXPECTED_CURRENT_RUNTIME='0d269e653bba534e7fc14591e2dc2dddb0373aedbddbe1284da7eeb33b980861'
INPUTS={'hermes-protected-registered.mjs': '7904085d39a50e3d2bbab9547ed2e230cc96a9d2592556d8bb5db722b8f6aaa8', 'hermes-harness-web-probe.mjs': 'e50661652246f65095602c61e4d215c3dbcd6056f6c49ef61d83ec12585a9751'}
...
def invoke(phase,args,candidate_view=False,protected=True):
 ...
 cmd=['systemd-run','--quiet','--wait','--pipe','--collect','--unit=himawari-loop-finalization-cutover-'+phase]
...
def main():
 assert os.geteuid()==0 and sys.flags.optimize==0 and len(sys.argv)==4 and sys.argv[1:3]==['--apply','--receipt']
 ...
 assert sha(Q/'hermes-loop-finalization-start.mjs')=='03c18c0cd4f6ae519a1288037883e53515c8fa89027070b52f5386a477eee1bf'
 ...
 publish('browser-session-policy.json',{'csrfTtlMilliseconds':json.loads(CONFIG.read_text())['identity']['csrf']['ttlMilliseconds']})
 ...
 try:
  status('checking_candidate_browser_before_stopping_service')
  assert json.loads(CONFIG.read_text())['http']['staticRoot']==str(CURRENT/'share/control-center'),'STATIC_ROOT_CONFIGURATION_CHANGED'
  web=invoke('web-static',[node,W/'hermes-harness-web-probe.mjs'],True)
  assert web['passed'] and web['runtimeUid']==998 and web['staticFiles']==6
  publish('web-static-preflight.json',web)
  ...
  startup=invoke('prepare',[node,Q/'hermes-loop-finalization-start.mjs','--prepare',prepared],True);assert startup['preparing'];assert startup['runtimeDigest']==qualified['runtimeDigest']
  ...
  authority_next=AUTH.with_name('runtime.harness-next.json');shutil.copyfile(CANDIDATE_AUTH,authority_next);authority_next.chmod(0o644);os.replace(authority_next,AUTH)
  os.replace(prepared,CONFIG)
  next_unit=UNIT.with_name('himawari.service.harness-next');next_unit.write_text(original_unit.replace(old_exec,'ExecStart='+str(node)+' '+str(Q/'hermes-loop-finalization-start.mjs')));next_unit.chmod(0o644);os.replace(next_unit,UNIT)
  ...
  status('starting_harness_lifecycle_service');new_started=True;command(['systemctl','start','himawari.service'])
  ...
  # The qualification step started the separately bounded read-only observer.
...
if __name__=='__main__':
 ...
 assert re.fullmatch('[0-9a-f]{64}',EXPECTED_RECEIPT),'QUALIFICATION_RECEIPT_REQUIRED'
