...
B=R/'builds/2026-09-12-harness-lifecycle'
Q=R/'qualifications/2026-09-12-harness-lifecycle-installation'
W=R/'qualifications/2026-09-12-harness-lifecycle-cutover'
CURRENT=R/'releases/2026-09-11-control-center'
CANDIDATE=R/'releases/2026-09-12-harness-lifecycle'
ARCHIVE=R/'releases/2026-09-12-before-harness-lifecycle'
...
CANDIDATE_AUTH=pathlib.Path('/etc/himawari/harness-lifecycle-candidate.json')
BACKUP_ID='before-harness-lifecycle-2026-09-12'
ATTESTATION=STATE/'deployment/startup-attestation.json'
OLD_START=R/'qualifications/2026-09-12-three-fixes-installation/hermes-three-fixes-start.mjs'
EXPECTED_RECEIPT=sys.argv[3] if len(sys.argv)==4 and sys.argv[1:3]==['--apply','--receipt'] else ''
EXPECTED_CURRENT_RUNTIME='a83239f95583c9c82264059c40e9f59bfddc5355017caedb62affedf4b441a56'
...
def invoke(phase,args,candidate_view=False,protected=True):
 ...
 cmd=['systemd-run','--quiet','--wait','--pipe','--collect','--unit=himawari-harness-cutover-'+phase]
...
def main():
 ...
 assert sha(Q/'hermes-harness-start.mjs')=='a56fe8ebf309874f839811b933161ef5495dc15552d8df586ef45878df7df46a'
 ...
 try:
  ...
  startup=invoke('prepare',[node,Q/'hermes-harness-start.mjs','--prepare',prepared],True);assert startup['preparing'];assert startup['runtimeDigest']==qualified['runtimeDigest']
  ...
  next_unit=UNIT.with_name('himawari.service.harness-next');next_unit.write_text(original_unit.replace(old_exec,'ExecStart='+str(node)+' '+str(Q/'hermes-harness-start.mjs')));next_unit.chmod(0o644);os.replace(next_unit,UNIT)
