...
const root = "/data/hermes/himawari",
  ...
  q = root + "/qualifications/2026-09-13-loop-finalization-installation/seal-stage";
process.umask(0o077);
...
for (const p of [
  ...
  qualificationSources[p] = await fileHash(
    root + "/qualifications/2026-09-13-loop-finalization-installation/probe-source/" + p,
  );
const receipt = {
  ...
  source: JSON.parse(
    await readFile(
      root + "/qualifications/2026-09-13-loop-finalization-installation/source-manifest.json",
      "utf8",
    ),
  ),
};
