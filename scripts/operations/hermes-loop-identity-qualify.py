...
INPUTS={'hermes-loop-identity-observer.py': '8b9feb0cc2f566e1db47f1f8822b6eb9de1c6ec076a8da2b064ef281fd7ad9ac', 'hermes-loop-identity-seal.mjs': 'a2fce34cf5dd15ac5e683ee108ec5bda3e11067b60b9cab8d06efb6aefee2be2', 'hermes-loop-identity-start.mjs': '4e058cb50e18be3a55508b40615b8b9c1b7b86aa3d1255b9983dcc651c9b41f0', 'probe-protected-runtime.mjs': '8d3c6cc8270dc79d14c1fb97988f1f0dcba97572b623ca49e6cbe1233b92d4de', 'hermes-harness-web-probe.mjs': 'f1bac69509ce6e06460978a603412ef6b2b7376c89021413557383ac2c099b5b'}
...
def run_user(name,phase,args,env=None,signer_view=False):
 ...
 cmd=['systemd-run','--quiet','--wait','--pipe','--collect','--unit=himawari-loop-identity-qualify-'+phase]
...
def main():
 ...
 assert json.loads((R/'qualifications/2026-09-13-loop-finalization-cutover/result.json').read_text())['deploymentComplete']
 assert json.loads(AUTH.read_text())['runtimeDigest']=='7b05555bbb12cb281d182c3c91d809f40b1dad2ac6c0e466799a051d2ffe57bd','CURRENT_INSTALLATION_CHANGED'
 assert sha(B/'preparation.json')=='589a941d6761fbe7d62c54e96a4ebfb335641b501b0387c169eedea8aeff27c7','PREPARATION_CHANGED'
 prepared=json.loads((B/'preparation.json').read_text())
 assert prepared['buildPassed'] and prepared['sourceArchiveSha256']=='af6cc7afaddde4ef82e77731bccd99c29a44d25ed723899df94cdffb00f7454e'
 ...
 assert sha(B/'himawari-loop-identity-source.json')=='cb48117fd67237bc16746b574bbcaf2da8aefc2ad38ff421d005db8757c72f39','SOURCE_MANIFEST_CHANGED'
 manifest=json.loads((B/'himawari-loop-identity-source.json').read_text())
 assert sha(B/'himawari-loop-identity-source.tar.gz')==manifest['archiveSha256'],'SOURCE_ARCHIVE_CHANGED'
 assert manifest['archiveSha256']=='af6cc7afaddde4ef82e77731bccd99c29a44d25ed723899df94cdffb00f7454e'
 ...
 try:
  ...
  shutil.copyfile(B/'himawari-loop-identity-source.json',Q/'source-manifest.json');(Q/'source-manifest.json').chmod(0o644)
  ...
  signer=run_user('andy','signer-preflight',[RUNTIME/'pi-tools/bin/node',Q/'hermes-loop-identity-seal.mjs','--preflight'],signer_view=True)
  ...
  result=run_user('andy','seal',[RUNTIME/'pi-tools/bin/node',Q/'hermes-loop-identity-seal.mjs'],signer_view=True)
  ...
  reader=runpy.run_path(str(Q/'hermes-loop-identity-observer.py'))
  ...
  subprocess.run(['systemd-run','--unit=himawari-loop-identity-live-observer','--property=RuntimeMaxSec=3700','/usr/bin/python3','-I',Q/'hermes-loop-identity-observer.py'],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
