...
INPUTS={'hermes-loop-finalization-observer.py': '4b46b099cf021eaf63e9965a5f6dc52de682fa5982f26d3ff20bd73a276ac7e4', 'hermes-loop-finalization-seal.mjs': '154ca759d740d49d17cdd649ab48b83c80f4d3335bcd28f40a885ab53a2ac20e', 'hermes-loop-finalization-start.mjs': '03c18c0cd4f6ae519a1288037883e53515c8fa89027070b52f5386a477eee1bf', 'probe-protected-runtime.mjs': '8d3c6cc8270dc79d14c1fb97988f1f0dcba97572b623ca49e6cbe1233b92d4de', 'hermes-harness-web-probe.mjs': 'e50661652246f65095602c61e4d215c3dbcd6056f6c49ef61d83ec12585a9751'}
...
def run_user(name,phase,args,env=None,signer_view=False):
 ...
 cmd=['systemd-run','--quiet','--wait','--pipe','--collect','--unit=himawari-loop-finalization-qualify-'+phase]
...
def main():
 ...
 assert json.loads((R/'qualifications/2026-09-12-harness-lifecycle-cutover/result.json').read_text())['deploymentComplete']
 assert json.loads(AUTH.read_text())['runtimeDigest']=='0d269e653bba534e7fc14591e2dc2dddb0373aedbddbe1284da7eeb33b980861','CURRENT_INSTALLATION_CHANGED'
 assert sha(B/'preparation.json')=='914a5a51ab4e64fb6abf960e64bfd79fd0b1c7731c75f5523018bf57a807911d','PREPARATION_CHANGED'
 prepared=json.loads((B/'preparation.json').read_text())
 assert prepared['buildPassed'] and prepared['sourceArchiveSha256']=='e97a997e6d71533db192e3b3ec2c89dc5ab4e7ac65c2e74583daff9f03a9943a'
 ...
 assert sha(B/'himawari-loop-finalization-source.json')=='99c020b0567bdc146bc1edfafc8f6b0d31833849f28b9c0aeaa3c7d54242ddf9','SOURCE_MANIFEST_CHANGED'
 manifest=json.loads((B/'himawari-loop-finalization-source.json').read_text())
 assert sha(B/'himawari-loop-finalization-source.tar.gz')==manifest['archiveSha256'],'SOURCE_ARCHIVE_CHANGED'
 assert manifest['archiveSha256']=='e97a997e6d71533db192e3b3ec2c89dc5ab4e7ac65c2e74583daff9f03a9943a'
 ...
 try:
  ...
  shutil.copyfile(B/'himawari-loop-finalization-source.json',Q/'source-manifest.json');(Q/'source-manifest.json').chmod(0o644)
  ...
  signer=run_user('andy','signer-preflight',[RUNTIME/'pi-tools/bin/node',Q/'hermes-loop-finalization-seal.mjs','--preflight'],signer_view=True)
  ...
  result=run_user('andy','seal',[RUNTIME/'pi-tools/bin/node',Q/'hermes-loop-finalization-seal.mjs'],signer_view=True)
  ...
  reader=runpy.run_path(str(Q/'hermes-loop-finalization-observer.py'))
  ...
  subprocess.run(['systemd-run','--unit=himawari-loop-finalization-live-observer','--property=RuntimeMaxSec=3700','/usr/bin/python3','-I',Q/'hermes-loop-finalization-observer.py'],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
