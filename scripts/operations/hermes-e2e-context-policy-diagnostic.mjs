...
const runs = [
  "run:ea16b5bc-b2ac-45f1-b0cd-1650cc749835",
  "run:3707de79-f306-4944-aaa7-a5390b26167c",
  "run:a2b3bb7a-7db8-4d97-94d2-7899ca4a7965",
  "run:9e9a2eb7-3466-4f09-bfb5-bd6699f01992",
  "run:5a57158b-e188-4f26-b763-9bc385af23e9",
];
const sha = (s) => createHash("sha256").update(s).digest("hex");
const markers = [
  "The owner selected answer language en",
  "The owner selected answer language ja",
  "Write user-facing answers in English",
  "Write user-facing answers in Japanese",
  "请使用中文",
  "默认使用简体中文",
  "使用简体中文帮助用户",
  "使用简体中文交流",
...
export function policyFlags(value) {
  const text = contentText(value);
  return {
    chineseDirective: /(?:使用|用|以|请|必须|默认)(?:简体)?中文(?:帮助|交流|回答|回复|输出)/.test(
      text,
    ),
    englishDirective: text.includes("Write user-facing answers in English"),
    japaneseDirective: text.includes("Write user-facing answers in Japanese"),
  };
}
...
async function main() {
  ...
  const report = {
    ...
    modelCalls: 0,
    runs: [],
    configuredSystemPolicy: summarize(config.runPolicy.systemInstruction),
  };
  try {
    for (const runId of runs) {
      ...
      for (const a of db
        .prepare(
          "SELECT operation_key,payload_ref FROM run_payload_artifacts WHERE owner_id=? AND agent_id=? AND run_id=? AND purpose='context'",
        )
        .all(...scope, runId)) {
        const value = await payload(a.payload_ref);
        item.contextArtifacts.push({
      ...
      for (const e of db
        ...
        .all(...scope, runId)) {
        if (e.payload_ref && e.event_type === "runtime.tool_result") {
          const result = await payload(e.payload_ref);
  ...
  const out = `${root}/qualifications/2026-09-12-e2e-context-policy-diagnostic`;
