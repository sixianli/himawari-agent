...
B=R/'builds/2026-09-12-ui-session'
Q=R/'qualifications/2026-09-12-ui-session-installation'
W=R/'qualifications/2026-09-12-ui-session-cutover'
CURRENT=R/'releases/2026-09-11-control-center'
CANDIDATE=R/'releases/2026-09-12-ui-session'
ARCHIVE=R/'releases/2026-09-12-before-ui-session'
...
CANDIDATE_AUTH=pathlib.Path('/etc/himawari/ui-session-candidate.json')
BACKUP_ID='before-ui-session-2026-09-12'
ATTESTATION=STATE/'deployment/startup-attestation.json'
OLD_START=R/'qualifications/2026-09-12-e2e-fixes-installation/hermes-e2e-fixes-start.mjs'
EXPECTED_RECEIPT='2de542e9a1110be5e0a95a67933941455f1aeff6cfb81605e98321d6f537e523'
EXPECTED_CURRENT_RUNTIME='c55fff099c0a25071e53586b2bb234858a2e2541046eff3c4390c3406c51bd02'
INPUTS={'hermes-ui-session-observer.py': 'c91099438aea1dd0412f697c0e1add84917adae010c078337528edd9742a6895', 'hermes-protected-registered.mjs': '7904085d39a50e3d2bbab9547ed2e230cc96a9d2592556d8bb5db722b8f6aaa8'}
...
def invoke(phase,args,candidate_view=False,protected=True):
 ...
 cmd=['systemd-run','--quiet','--wait','--pipe','--collect','--unit=himawari-ui-session-cutover-'+phase]
...
def main():
 ...
 assert sha(Q/'hermes-ui-session-start.mjs')=='417c60f4f2a95710c645e5b0db07ce05b1dd7b6b1e2bcafcbf7d87fc29d990a7'
 ...
 try:
  ...
  startup=invoke('prepare',[node,Q/'hermes-ui-session-start.mjs','--prepare',prepared],True);assert startup['preparing'];assert startup['runtimeDigest']==qualified['runtimeDigest']
  ...
  authority_next=AUTH.with_name('runtime.ui-session-next.json');shutil.copyfile(CANDIDATE_AUTH,authority_next);authority_next.chmod(0o644);os.replace(authority_next,AUTH)
  os.replace(prepared,CONFIG)
  next_unit=UNIT.with_name('himawari.service.ui-session-next');next_unit.write_text(original_unit.replace(old_exec,'ExecStart='+str(node)+' '+str(Q/'hermes-ui-session-start.mjs')));next_unit.chmod(0o644);os.replace(next_unit,UNIT)
  ...
  status('starting_ui_session_service');new_started=True;command(['systemctl','start','himawari.service'])
  ...
  try:
   command(['systemd-run','--unit=himawari-ui-session-live-observer','--property=RuntimeMaxSec=3700','/usr/bin/python3','-I',W/'hermes-ui-session-observer.py'])
...
if __name__=='__main__':
 ...
 fd=os.open('/run/himawari-ui-session-cutover.lock',os.O_CREAT|os.O_RDWR|os.O_NOFOLLOW,0o600)
