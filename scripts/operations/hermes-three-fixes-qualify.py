#!/usr/bin/python3
"""Qualify the Pi-guidance candidate in a private view; keep the live service untouched."""
...
Q=R/'qualifications/2026-09-12-three-fixes-installation'
CANDIDATE=R/'releases/2026-09-12-three-fixes'
...
CANDIDATE_AUTH=pathlib.Path('/etc/himawari/three-fixes-candidate.json')
SCRATCH=pathlib.Path('/data/himawari-r8-protected')
HERE=pathlib.Path(__file__).resolve().parent
INPUTS={'hermes-three-fixes-observer.py': '892ae2ef4f8dbbbf5286b51a8ca813c0a8ac1235751b0f3dfe8a2a1153860c3f', 'hermes-three-fixes-seal.mjs': '2bdc36a7b65d9a691ddbc17cfc6f4e516034e329615ea93e7b841c6dc1e79fa7', 'hermes-three-fixes-start.mjs': '363fcfdc19699b2c51b2b08986bd8ba1313c813a9310df69506d55e5bd7d2808', 'probe-protected-runtime.mjs': '8d3c6cc8270dc79d14c1fb97988f1f0dcba97572b623ca49e6cbe1233b92d4de'}
...
def run_user(name,phase,args,env=None,signer_view=False):
 ...
 cmd=['systemd-run','--quiet','--wait','--pipe','--collect','--unit=himawari-three-fixes-qualify-'+phase]
...
def main():
 ...
 assert json.loads((R/'qualifications/2026-09-12-ui-session-cutover/result.json').read_text())['deploymentComplete']
 assert json.loads(AUTH.read_text())['runtimeDigest']=='45dd3fa8dbb27b796e51865cfed7ceb871589f42a3b6ce2ca59c5bf362510d7f','CURRENT_INSTALLATION_CHANGED'
 assert sha(B/'preparation.json')=='0a8581ae8cca71ce4dcc12f59511c4d08bdad9c49c57c0ff68b8cbdbaa537ea6','PREPARATION_CHANGED'
 prepared=json.loads((B/'preparation.json').read_text())
 assert prepared['buildPassed'] and prepared['sourceArchiveSha256']=='a475f185af58e19fb4d6c53e933cc2b111db62e26c843c876daa0d60a423458f'
 ...
 assert sha(B/'himawari-three-fixes-source.json')=='b647db093f59db54a97a427f5c0fe5bf8a1006a2b1fee1bf91d0b635b1a614c8','SOURCE_MANIFEST_CHANGED'
 manifest=json.loads((B/'himawari-three-fixes-source.json').read_text())
 assert sha(B/'himawari-three-fixes-source.tar.gz')==manifest['archiveSha256'],'SOURCE_ARCHIVE_CHANGED'
 assert manifest['archiveSha256']=='a475f185af58e19fb4d6c53e933cc2b111db62e26c843c876daa0d60a423458f'
 ...
 try:
  ...
  shutil.copyfile(B/'himawari-three-fixes-source.json',Q/'source-manifest.json');(Q/'source-manifest.json').chmod(0o644)
  ...
  signer=run_user('andy','signer-preflight',[RUNTIME/'pi-tools/bin/node',Q/'hermes-three-fixes-seal.mjs','--preflight'],signer_view=True)
  ...
  result=run_user('andy','seal',[RUNTIME/'pi-tools/bin/node',Q/'hermes-three-fixes-seal.mjs'],signer_view=True)
  ...
  reader=runpy.run_path(str(Q/'hermes-three-fixes-observer.py'))
  ...
  subprocess.run(['systemd-run','--unit=himawari-three-fixes-live-observer','--property=RuntimeMaxSec=3700','/usr/bin/python3','-I',Q/'hermes-three-fixes-observer.py'],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
