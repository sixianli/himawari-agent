...
const out = `${root}/qualifications/2026-09-13-harness-finalization-comparison`;
const runId = "run:cb547c58-80ea-42e6-8b90-4cfac81dcdf2";
const modelId = "deepseek/deepseek-v4-flash-0731";
const hash = (value) => createHash("sha256").update(value).digest("hex");
// Model-independent experiment: preserve all messages and remove active tools only.
// Pi 0.84.2 serializes context.tools=[] as tools:[] when tool history is present.
...
async function main() {
  ...
  const unpriced = sources.flatMap((source) =>
    finalizationInputs(source.original).map((input) => ({
      ...input,
  ...
  const report = {
    ...
    maximumCostMicros,
    maximumCalls: 8,
    experiment: "native Pi empty active tools after a complete four-tool batch",
  ...
  console.log("Finalization comparison written: " + out + "/summary.json");
}
