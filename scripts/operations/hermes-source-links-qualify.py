...
B=R/'builds/2026-09-13-source-links'
Q=R/'qualifications/2026-09-13-source-links-installation'
CANDIDATE=R/'releases/2026-09-13-source-links'
...
CANDIDATE_AUTH=pathlib.Path('/etc/himawari/source-links-candidate.json')
SCRATCH=pathlib.Path('/data/himawari-r8-protected')
HERE=pathlib.Path(__file__).resolve().parent
INPUTS={'hermes-source-links-observer.py': '83fc8c883286cead44b76d996a07d20d2087df7605fa8b22ea036d8d2a3f6b85', 'hermes-source-links-seal.mjs': 'd0edd23a3ff10c92135debeebc7eaeb04fd06db2117b8e8be9e08220de029249', 'hermes-source-links-start.mjs': 'bb2df0c1ec1b2d990eb3db00b74440c24e1982eb7e477a5a674dfaad309ee467', 'probe-protected-runtime.mjs': '8d3c6cc8270dc79d14c1fb97988f1f0dcba97572b623ca49e6cbe1233b92d4de', 'hermes-source-links-web-probe.mjs': '40bd188d2cb8560817ad665277e435c5abce442f3bcb2783943f31470a3bc510'}
...
def run_user(name,phase,args,env=None,signer_view=False):
 ...
 cmd=['systemd-run','--quiet','--wait','--pipe','--collect','--unit=himawari-source-links-qualify-'+phase]
...
def main():
 ...
 assert json.loads((R/'qualifications/2026-09-13-search-time-cutover/result.json').read_text())['deploymentComplete']
 assert json.loads(AUTH.read_text())['runtimeDigest']=='a9b48f6731f4087feff92f000c98f2436245ca580569708d8f585f4d848c307d','CURRENT_INSTALLATION_CHANGED'
 assert sha(B/'preparation.json')=='9984b7fc52fab862531296adc3dea35a17370eab6abb20d6a9162179bc81f42c','PREPARATION_CHANGED'
 prepared=json.loads((B/'preparation.json').read_text())
 assert prepared['buildPassed'] and prepared['sourceArchiveSha256']=='941bc302192483b1f3c2235b968da633f92ee52978298f735f35a3b35e29ef36'
 ...
 assert sha(B/'himawari-source-links-source.json')=='3c74c51176df85db22ce85dc0808b03da0b5b6f360ecdc30fc93e34c1eca4e41','SOURCE_MANIFEST_CHANGED'
 manifest=json.loads((B/'himawari-source-links-source.json').read_text())
 assert sha(B/'himawari-source-links-source.tar.gz')==manifest['archiveSha256'],'SOURCE_ARCHIVE_CHANGED'
 assert manifest['archiveSha256']=='941bc302192483b1f3c2235b968da633f92ee52978298f735f35a3b35e29ef36'
 ...
 try:
  ...
  shutil.copyfile(B/'himawari-source-links-source.json',Q/'source-manifest.json');(Q/'source-manifest.json').chmod(0o644)
  ...
  signer=run_user('andy','signer-preflight',[RUNTIME/'pi-tools/bin/node',Q/'hermes-source-links-seal.mjs','--preflight'],signer_view=True)
  ...
  web=run_user('himawari','web-static',[RUNTIME/'pi-tools/bin/node',Q/'hermes-source-links-web-probe.mjs'])
  ...
  assert digest=='a9b48f6731f4087feff92f000c98f2436245ca580569708d8f585f4d848c307d','UNEXPECTED_RUNTIME_CHANGE_FOR_BROWSER_ONLY_CANDIDATE'
  ...
  result=run_user('andy','seal',[RUNTIME/'pi-tools/bin/node',Q/'hermes-source-links-seal.mjs'],signer_view=True)
  ...
  reader=runpy.run_path(str(Q/'hermes-source-links-observer.py'))
  ...
  subprocess.run(['systemd-run','--unit=himawari-source-links-live-observer','--property=RuntimeMaxSec=3700','/usr/bin/python3','-I',Q/'hermes-source-links-observer.py'],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
