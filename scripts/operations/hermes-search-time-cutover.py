...
W=R/'qualifications/2026-09-13-search-time-cutover'
CURRENT=R/'releases/2026-09-11-control-center'
CANDIDATE=R/'releases/2026-09-13-search-time'
ARCHIVE=R/'releases/2026-09-13-before-search-time'
...
BACKUP_ID='before-search-time-2026-09-13'
ATTESTATION=STATE/'deployment/startup-attestation.json'
OLD_START=R/'qualifications/2026-09-13-loop-identity-installation/hermes-loop-identity-start.mjs'
EXPECTED_RECEIPT=sys.argv[3] if len(sys.argv)==4 and sys.argv[1:3]==['--apply','--receipt'] else ''
EXPECTED_CURRENT_RUNTIME='4c2ffd5de278c5b294860dcae87cbd64a826990a8cd596cb9ed16e84246ceb13'
...
def invoke(phase,args,candidate_view=False,protected=True):
 ...
 cmd=['systemd-run','--quiet','--wait','--pipe','--collect','--unit=himawari-search-time-cutover-'+phase]
...
def main():
 ...
 assert sha(Q/'hermes-search-time-start.mjs')=='32abd1350081c14e6a9937dc293aa9a0359c81646743f534d8b2e3f59581c817'
 ...
 try:
  ...
  startup=invoke('prepare',[node,Q/'hermes-search-time-start.mjs','--prepare',prepared],True);assert startup['preparing'];assert startup['runtimeDigest']==qualified['runtimeDigest']
  ...
  next_unit=UNIT.with_name('himawari.service.harness-next');next_unit.write_text(original_unit.replace(old_exec,'ExecStart='+str(node)+' '+str(Q/'hermes-search-time-start.mjs')));next_unit.chmod(0o644);os.replace(next_unit,UNIT)
