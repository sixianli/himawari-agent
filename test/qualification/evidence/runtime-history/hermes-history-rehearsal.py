...
Q=R/'qualifications/2026-09-11-native-history-rehearsal'
C=R/'qualifications/2026-09-11-protected/attempt-v5/native-history-import-candidate.json'
MODULE=pathlib.Path(__file__).with_name('rehearse-legacy-history.mjs')
MODULE_SHA='84d20044fe23adef86c99e4eee34daa741bc46343cd6405bceb53894f2e9be6b'
ARCHIVE_SHA='8c55f24545a717065632a17efd1f4aed9af264811cf98f505883b0c01f3fda8a'
NODE_SOURCE='''
import assert from 'node:assert/strict';
import {readFile} from 'node:fs/promises';
import {createRequire} from 'node:module';
import {rehearseLegacyHistory} from '/data/hermes/himawari/qualifications/2026-09-11-native-history-rehearsal/rehearse-legacy-history.mjs';
const root='/data/hermes/himawari', q=root+'/qualifications/2026-09-11-native-history-rehearsal';
...
def main():
 ...
 tested=json.loads((B/'rehearsal-self-test.json').read_text());assert tested['testsPassed']==3 and tested['positive']['passed'] and tested['sourceSchemaUnchanged']
 ...
 command=['systemd-run','--quiet','--wait','--pipe','--collect','--unit=himawari-native-history-rehearsal-child',
 ...
 str(R/'releases/2026-09-11-control-center/lib/himawari-agent/pi-tools/bin/node'),'--input-type=module']
 with (Q/'private.stderr').open('x') as error:
  result=subprocess.run(command,input=NODE_SOURCE,text=True,stdout=subprocess.PIPE,stderr=error,timeout=630)
 if result.returncode:
  private=(Q/'private.stderr').read_text(errors='replace')
  report={'passed':False,'exitCode':result.returncode,'errorCodes':sorted(set(re.findall(r'\b(?:IMPORT_[A-Z_]+|REHEARSAL_[A-Z_]+|SQLITE_[A-Z_]+|EACCES|EPERM|ENOENT)\b',private)))}
 else:
