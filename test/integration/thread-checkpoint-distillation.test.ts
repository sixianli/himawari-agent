...
describe.each(["worker", "direct"] as const)(
  "thread-checkpoint-distillation through %s",
  (execution) => {
    ...
    describe("Task 19 durable Thread checkpoints and distillation", () => {
      it("supports every governed trigger without changing Thread lifecycle and deduplicates identity", async () => {
        ...
        const database = openQualifiedDatabase(path.join(stateRoot, "product.sqlite"));
        expect(
          database.prepare("SELECT status FROM threads WHERE id = ?").pluck().get(THREAD_ID),
        ).toBe("open");
      ...
      it("recovers an expired process lease after restart and records qualification metrics", async () => {
        ...
        const secondaryText = restartedContent.plaintextByRef.get(
          secondarySummary?.contentRef ?? "",
        );
        const expectedFacts = [
        ...
        const sourceCoverage =
          [source(1).ref, source(2).ref, source(6).ref].filter((ref) => provenance.has(ref))
            .length / 3;
