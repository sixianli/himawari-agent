...
describe.each(["worker", "direct"] as const)("automatic-memory-golden through %s", (execution) => {
  ...
  class GoldenExtractor implements IncrementalMemoryExtractionPort {
    ...
    async extract(input: {
      ...
      return testCase.expectedCandidates.map(
        ({ expectedDurable: _expectedDurable, ...candidate }) => Object.freeze(candidate),
      );
    }
  }
