...
describe.each(["worker", "direct"] as const)(
  "durable-memory-projection through %s",
  (execution) => {
    ...
    describe("durable product Memory projection", () => {
      it.each(["create", "update", "correct", "archive", "delete"] as const)(
        ...
        async (operation) => {
          ...
          const before =
            operation === "create"
              ? undefined
              : await memory.applyProposal(proposal, GENERATION_ID);
      ...
      it("repairs legacy missing create and deletion jobs on replay without changing revision", async () => {
        ...
        try {
          ...
          database
            .prepare("DELETE FROM memory_projection_jobs WHERE memory_id = ?")
            .run(created.id);
          ...
          database
            .prepare(
              "DELETE FROM memory_projection_jobs WHERE memory_id = ? AND operation = 'delete'",
            )
            .run(created.id);
      ...
      it("commits independently, deduplicates proposals and skips stale projection revisions", async () => {
        ...
        expect((await resource.repository.productMemoryState().read(created.id))?.lastUsedAt).toBe(
          T0,
        );
    ...
    describe("Memory projection unified model budget", () => {
      it("requires the current projection claim and preserves unknown spend across retries", async () => {
        ...
        VALUES ('memory-authority', ?, ?, 'memory-deployment', 'memory-host', 1, 1, ?, '2999-01-01T00:00:00.000Z')`)
          ...
          await expect(
            budget.reserve({ ...input, operationKey: "embedding:retry" }),
