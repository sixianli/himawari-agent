...
describe.each(["worker", "direct"] as const)(
  "thread-deletion-coordination through %s",
  (execution) => {
    ...
    describe("Thread deletion coordination", () => {
      it("requires explicit task resolution and preserves independent authorization state", async () => {
        ...
        await expect(
          coordinator.inspect(ownerId, agentId, source.thread.id),
        ...
        await expect(noGuardCoordinator.deletePermanently(permanentInput)).resolves.toEqual(
          pending,
        );
