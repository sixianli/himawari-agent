...
describe.each(["worker", "direct"] as const)(
  "durable-background-recovery through %s",
  (execution) => {
    ...
    function recoveryScope(
      authority: ProductAuthorityFence = AUTHORITY,
    ...
    describe("Task 12 durable background recovery", () => {
      ...
      it("enforces persistent hard budgets, category capacity, and foreground reservation", async () => {
        ...
        const backgroundA = await service.dispatch(
          dispatchInput(
            occurrence({ id: "occurrence-a", jobId: "job-background-a" }),
            capacityLimits,
          ),
        );
        const backgroundB = await service.dispatch(
          dispatchInput(
            occurrence({ id: "occurrence-b", jobId: "job-background-b" }),
            capacityLimits,
          ),
        );
