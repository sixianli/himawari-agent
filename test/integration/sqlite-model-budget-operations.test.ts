...
// Run the same persistence contracts both through the production Worker transport
// and directly against the SQL operation layer. The latter makes its business
// branches observable to Vitest without treating Worker coverage as collected.
describe.each(["worker", "direct"] as const)("SQLite budget contracts (%s)", (execution) => {
  ...
  describe("SQLite model budget migration red tests", () => {
    ...
    it("rejects a Run budget parent when the Run is already owned by an occurrence", async () => {
      ...
      try {
        ...
        const created = await reverse.repository
          .backgroundWorkState()
  ...
  describe("SQLite durable model invocation identities", () => {
    it("revalidates current authority before replaying an identity", async () => {
      ...
      try {
        ...
        try {
          const staleIdentity = restarted.modelInvocationIdentityPort(
            OWNER_ID,
            AGENT_ID,
            AUTHORITY,
            {
          ...
          await expect(
            staleIdentity.begin(identityInput(fixtureState.claim)),
