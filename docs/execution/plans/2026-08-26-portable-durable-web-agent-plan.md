...
模型调用前逐条执行机器秘密扫描；summary 与非敏感候选先写稳定 protected Payload，敏感或 restricted 候选只提交无正文的 `awaiting_sensitive_approval` metadata。`pre_compaction` 现在要求 Pi 已保护 summary 作为既有 `summary_ref`，distillation model 必须返回空 summary 并只提取 candidates；完成事务原样发布该引用，不再生成或存储第二份摘要。其他三类 trigger 仍生成受保护 summary。SQLite 在单个 immediate transaction 中共同写 summary、全部 candidates/provenance 及 completed 标记；事务前失败不可见，COMMIT 后回包丢失按 generation identity 回读原输出。未解决 commitment 不连接 Scheduler、Capability 或 external action。Context Formation 只附加符合分类限制的最新摘要，原始 transcript 仍完整保存并继续注入。
...
- [x] 在第一次 live call 前向 Owner 展示 primary、fixed fallback 和 embedding 的精确 provider/model/version 或 snapshot、能力、披露、费用、secret ref、预计成本和 capped test budget。
...
当前实现冻结 strict primary/fallback generation descriptor 和独立 embedding descriptor：生成模型配置显式包含 priority、能力、输入模态、上下文、最大输出与四项费用，embedding 显式包含 provider/model/version、dimensions、能力、披露、费用与 secret ref，并强制与 Mem0 vector dimension 相等。Router 先走 primary，retryable provider failure 才能走 fixed fallback，并对重复 output sequence 与 terminal result 去重。Owner 已指定本次主要生成模型候选为 OpenRouter `deepseek/deepseek-v4-flash-0731`，补充指定 fallback 模型候选为 OpenRouter `z-ai/glm-5.3-flash`，并批准本次 bounded live smoke/eval 的总费用上限为 `1.00 USD`。primary 与 fallback 共用同一个 OpenRouter API key，凭据通过 macOS Keychain 的独立 opaque host secret source 解析，不复用固定大小的 Payload 加密密钥 source。Owner 已确认本次启用真实 fallback，但 fallback 只允许接收 `private` 数据；OpenRouter 的 disclosure 仍为 `external_remote`，并已确认 fallback 的 provider routing 为 `order: ["z-ai"]`、`allow_fallbacks: false`、`require_parameters: true`、`data_collection: deny`；primary 不强制该上游顺序。2026-08-27 对两个 OpenRouter 模型页的只读核对已记录 generation catalog snapshot；2026-08-28 Owner 又确认 OpenRouter `qwen/qwen3-embedding-8b`、本地 catalog snapshot `catalog-2026-08-28`、`4096` dimensions、`external_remote` disclosure、`0.01 USD/M input tokens` 和共享 `openrouter-api-key@v1`，并完成仅公开合成文本的 Mem0 live embedding smoke（3 次请求、21 tokens、按 catalog 估算 `0.00000021 USD`）。OpenRouter/Qwen 页面没有提供不可变模型版本，因此日期化 snapshot 不冒充 upstream immutable version。

改造后只保留一份 canonical `ConfiguredPiModelDescriptor`，不再配对维护可能漂移的产品/Pi generation 双描述符；embedding 不进入 Pi，因为当前 Pi Mono 版本没有可复用的 embedding runtime。`resolveConfiguredModelDescriptorSet()` 从严格 ProductConfiguration 解析 secret version/purpose，把获准的 OpenRouter primary/fallback 映射为 Pi descriptor，同时返回独立、带 dimensions/cost/disclosure 的 embedding descriptor。`ConfiguredPiModelBindingPort` 使用 Pi `InMemoryCredentialStore`，关闭磁盘模型发现和网络刷新，只注册两个显式模型。`PiModelTransport` 把 provider HTTP、SSE parsing、usage 与模型运行交给 Pi `ModelRuntime.stream()`；产品边界只做 protected Payload、Secret Handle、跨 chunk secret redaction 和 Pi `0.84.2` 缺失的 OpenRouter provider/generation/actual cost 旁路观察。Memory 组合复用 `mem0ai/oss@3.1.7` 的 OpenAI-compatible embedder，把 strict embedding descriptor 的模型和 `4096` dimensions 映射为 `baseURL=https://openrouter.ai/api/v1`、`provider="openai"`、`embeddingDims=4096`，不新增第二套 embedding HTTP/vector/history 实现。`PiAgentRuntimeAdapter` 以结构化消息预填充 `SessionManager`，实时产出事件，并只从产品授权 additional paths 加载资源。`apps/agent-service` 的 production `main` 现在在支持的 provider 配置下创建并在 drain 时关闭 Model/Pi 与 Mem0 composition；deterministic fixture 只生成 descriptor-only 诊断，不创建 Pi、Mem0 或隐藏模型。确定性部分以及 embedding identity 展示/批准和有界 embedding live smoke 已完成；generation live provider 身份、费用、模型质量、cancellation/fallback/tool-call/disclosure/redaction 的完整 live evidence 以及最终 Gateway/Memory/Worker 总组合仍保持未验证。
...
| 模型路由 | Task 20 | 精确 descriptors、Owner 费用授权、deterministic 与有界 live evidence；证据：Task 20 model-routing evidence | 部分验证：canonical generation descriptor、独立 4096 维 Qwen embedding descriptor、primary/fixed fallback、disclosure、Mem0 OpenAI-compatible projection、Pi ModelRuntime transport、secret redaction、provider metadata observation、provider routing、strict configuration、闭合 Pi 模型绑定和 Agent Service entrypoint 生命周期接入通过；embedding live provider/model/vector/token/cost readback 已记录；generation catalog snapshot 已记录但官方没有 immutable version，generation live identity、费用、eval 以及 cancellation/fallback/tool-call/disclosure/redaction 组合仍未验证 |
...
本轮 fresh evidence 已覆盖模型确定性边界、4096 维 Qwen embedding 的 Mem0 production projection 与有界 live readback、GitHub 确定性边界、重复结果恢复、浏览器 disclosure preview、本机规模切片和安装后服务；仍缺少 generation paid/live model、真实 GitHub/Cloudflare、Safari/Firefox/真实移动设备、Mac↔Hermes transfer、immutable clean-install 的双平台 readback、各验收项的外部 readback 证据、完整跨平台矩阵和 7 天 soak。因此 Task 30 与本 Plan 收口清单保持未完成，不把 embedding live success 推断为完整生产验收。
