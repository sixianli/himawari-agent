...
当前实现冻结 strict primary/fallback/embedding descriptor 与 disclosure/secret ref 边界；Router 先走 primary，retryable provider failure 才能走 fixed fallback，并对重复 output sequence 与 terminal result 去重。Owner 已指定本次主要生成模型候选为 OpenRouter `deepseek/deepseek-v4-flash-0731`；本地 `OpenRouterModelTransport` 已接入受保护 Payload boundary，使用 SSE streaming、实际 usage/cost 与 provider metadata，并在跨 chunk 输出持久化前执行 machine-secret redaction；provider API key 使用独立的 opaque host secret source，不复用固定大小的 Payload 加密密钥 source。fallback、embedding 的最终 descriptor、provider routing、API key source reference 和 capped test budget 尚未冻结；因此本 Task 的付费 live call、费用、真实 provider 身份与模型质量仍保持未验证。确定性证据位于 `test/integration/qualification/evidence/s1-task20-model-routing.json`。
