...
当前实现冻结 strict primary/fallback/embedding descriptor 与 disclosure/secret ref 边界；Router 先走 primary，retryable provider failure 才能走 fixed fallback，并对重复 output sequence 与 terminal result 去重。Owner 已指定本次主要生成模型候选为 OpenRouter `deepseek/deepseek-v4-flash-0731`，补充指定 fallback 模型候选为 OpenRouter `z-ai/glm-5.3-flash`，并批准本次 bounded live smoke/eval 的总费用上限为 `1.00 USD`；本地 `OpenRouterModelTransport` 已接入受保护 Payload boundary，使用 SSE streaming、实际 usage/cost 与 provider metadata，并在跨 chunk 输出持久化前执行 machine-secret redaction；primary 与 fallback 共用同一个 OpenRouter API key，凭据通过 macOS Keychain 的独立 opaque host secret source 解析，不复用固定大小的 Payload 加密密钥 source。Owner 已确认本次启用真实 fallback，但 fallback 只允许接收 `private` 数据；OpenRouter 的 disclosure 仍为 `external_remote`，并已确认 fallback 的 provider routing 为 `order: ["z-ai"]`、`allow_fallbacks: false`、`require_parameters: true`、`data_collection: deny`；primary 不强制该上游顺序，以免把 fallback 专属 provider 规则错误应用到 primary。fallback 的 deterministic/fault-injection 覆盖继续保留。fallback 使用的 Keychain account/service 已完成非空 readback。2026-08-27 对两个 OpenRouter 模型页的只读核对已记录 primary/fallback 的 catalog snapshot、能力、上下文、最大输出和价格；官方页面没有提供不可变模型版本，因此日期化 snapshot 不冒充 upstream immutable version。新增的 `ConfiguredPiModelBindingPort` 关闭 Pi 磁盘模型发现和网络刷新，只注册这两个显式模型，按需从 production-suitable host secret source 解析共享 provider secret；`createProductionModelComposition()` 已把同一组 descriptor 接入受保护 OpenRouter transport、TrustedModelProviderAdapter 和 Pi binding，并通过 strict configuration、unit 与 Pi 0.84.2 compatibility tests。当前仍缺 embedding descriptor、最终 Agent Service entrypoint 对该 model path 的接入和 live evidence；因此本 Task 的付费 live call、费用、真实 provider 身份与模型质量仍保持未验证。确定性证据位于 `test/integration/qualification/evidence/s1-task20-model-routing.json`。
...
| 模型路由 | Task 20 | 精确 descriptors、Owner 费用授权、deterministic 与有界 live evidence；证据：Task 20 model-routing evidence | 部分验证：descriptor、primary/fixed fallback、disclosure、OpenRouter transport、secret redaction、duplicate result、provider routing、strict configuration、闭合 Pi 模型绑定和模型路径组合通过；generation catalog snapshot 已记录但官方没有 immutable version，仍待 embedding descriptor、最终 Agent Service entrypoint 接入及 live provider 身份/费用/eval |
