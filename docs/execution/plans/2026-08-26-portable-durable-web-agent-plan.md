...
- [x] 获批后运行有界 live smoke/eval，并与 deterministic recovery、cancellation、tool-call、fallback、cost accounting 和 secret-redaction tests 一起记录证据。

当前实现冻结 strict primary/fallback generation descriptor 和独立 embedding descriptor：生成模型配置显式包含 priority、能力、输入模态、上下文、最大输出与四项费用，embedding 显式包含 provider/model/version、dimensions、能力、披露、费用与 secret ref，并强制与 Mem0 vector dimension 相等。Router 先走 primary，retryable provider failure 才能走 fixed fallback，并对重复 output sequence 与 terminal result 去重；Pi transport 现把 token 截断和零正文分别映射为 `OPENROUTER_OUTPUT_TRUNCATED` 与 `OPENROUTER_EMPTY_RESPONSE`，不能把无可用正文误报为 completed。Owner 已指定本次主要生成模型为 OpenRouter `deepseek/deepseek-v4-flash-0731`，fallback 为 OpenRouter `z-ai/glm-5.3-flash`，并批准 bounded live smoke/eval 的总费用上限为 `1.00 USD`。两者共用 macOS Keychain 中的 opaque OpenRouter secret ref；fallback 只允许接收 `private` 数据，并固定 provider routing 为 `order: ["z-ai"]`、`allow_fallbacks: false`、`require_parameters: true`、`data_collection: deny`。2026-08-28 fresh 目录核对把 generation snapshot 统一为 `catalog-2026-08-28`，确认两个模型的上下文和最大 completion 均为 `1,310,720 / 131,072` tokens；页面仍未提供不可变 upstream version，因此日期化 snapshot 不冒充官方 immutable version。Qwen embedding live smoke 继续确认 4096 维、3 次请求、21 tokens 和 catalog 估算 `0.00000021 USD`。

改造后只保留一份 canonical `ConfiguredPiModelDescriptor`，不再配对维护可能漂移的产品/Pi generation 双描述符；embedding 不进入 Pi，因为当前 Pi Mono 版本没有可复用的 embedding runtime。`resolveConfiguredModelDescriptorSet()` 从严格 ProductConfiguration 解析 secret version/purpose，把获准的 OpenRouter primary/fallback 映射为 Pi descriptor，同时返回独立、带 dimensions/cost/disclosure 的 embedding descriptor。`ConfiguredPiModelBindingPort` 使用 Pi `InMemoryCredentialStore`，关闭磁盘模型发现和网络刷新，只注册两个显式模型。`PiModelTransport` 把 provider HTTP、SSE parsing、usage 与模型运行交给 Pi `ModelRuntime.stream()`；产品边界只做 protected Payload、Secret Handle、跨 chunk secret redaction、稳定失败终态和 Pi `0.84.2` 缺失的 OpenRouter provider/generation/actual cost 旁路观察。Memory 组合复用 `mem0ai/oss@3.1.7` 的 OpenAI-compatible embedder，不新增第二套 embedding HTTP/vector/history 实现。`PiAgentRuntimeAdapter` 以结构化消息预填充 `SessionManager`，实时产出事件，并只从产品授权 additional paths 加载资源。2026-08-28 的有界 generation live qualification 先让 primary 正常调用，再在本地注入一次 retryable 503 触发真实 fixed fallback；最终 primary 由 `Makora`、fallback 由 `Z.AI` 执行，两者均返回预期正文，精确 generation/model/provider/tokens/cost 已回读，成功运行合计 `0.000026 USD`。deterministic suites 同时覆盖 cancellation、tool-call 拒绝、fallback、disclosure、secret redaction、重复终态、空响应和截断；Task 20 因此完成。最终 Gateway/Memory/Worker 总组合仍属于 Tasks 27–30，不由本 Task 的有界 provider 资格代替。
