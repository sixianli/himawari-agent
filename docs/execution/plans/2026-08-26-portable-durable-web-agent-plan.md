...
当前实现冻结 strict primary/fallback descriptor，以及 embedding 必须使用独立 descriptor 的边界；Router 先走 primary，retryable provider failure 才能走 fixed fallback，并对重复 output sequence 与 terminal result 去重。Owner 已指定本次主要生成模型候选为 OpenRouter `deepseek/deepseek-v4-flash-0731`，补充指定 fallback 模型候选为 OpenRouter `z-ai/glm-5.3-flash`，并批准本次 bounded live smoke/eval 的总费用上限为 `1.00 USD`。primary 与 fallback 共用同一个 OpenRouter API key，凭据通过 macOS Keychain 的独立 opaque host secret source 解析，不复用固定大小的 Payload 加密密钥 source。Owner 已确认本次启用真实 fallback，但 fallback 只允许接收 `private` 数据；OpenRouter 的 disclosure 仍为 `external_remote`，并已确认 fallback 的 provider routing 为 `order: ["z-ai"]`、`allow_fallbacks: false`、`require_parameters: true`、`data_collection: deny`；primary 不强制该上游顺序。2026-08-27 对两个 OpenRouter 模型页的只读核对已记录 catalog snapshot、能力、上下文、最大输出和价格；官方页面没有提供不可变模型版本，因此日期化 snapshot 不冒充 upstream immutable version。

改造后只保留一份 canonical `ConfiguredPiModelDescriptor`，不再配对维护产品/Pi 两套描述符。`ConfiguredPiModelBindingPort` 使用 Pi `InMemoryCredentialStore`，关闭磁盘模型发现和网络刷新，只注册两个显式模型。`PiModelTransport` 把 provider HTTP、SSE parsing、usage 与模型运行交给 Pi `ModelRuntime.stream()`；产品边界只做 protected Payload、Secret Handle、跨 chunk secret redaction 和 Pi `0.84.2` 缺失的 OpenRouter provider/generation/actual cost 旁路观察。`PiAgentRuntimeAdapter` 以结构化消息预填充 `SessionManager`，实时产出事件，并只从产品授权 additional paths 加载资源。`createProductionModelComposition()` 已把同一 descriptors 接入 TrustedModelProviderAdapter 和 Pi binding。当前仍缺 embedding descriptor、最终 Agent Service entrypoint 接入和 live evidence；因此付费调用、真实 provider 身份/费用和模型质量仍保持未验证。原 Task 20 evidence 记录的是改造前 deterministic baseline，本次 Pi-first 改造由当前代码、unit/Pi compatibility 与 fresh check 证明，尚未改写历史 evidence 文件。
...
| 模型路由 | Task 20 | 精确 descriptors、Owner 费用授权、deterministic 与有界 live evidence；证据：Task 20 model-routing evidence | 部分验证：canonical descriptor、primary/fixed fallback、disclosure、Pi ModelRuntime transport、secret redaction、provider metadata observation、provider routing、strict configuration、闭合 Pi 模型绑定和模型路径组合通过；generation catalog snapshot 已记录但官方没有 immutable version，仍待 embedding descriptor、最终 Agent Service entrypoint 接入及 live provider 身份/费用/eval |
