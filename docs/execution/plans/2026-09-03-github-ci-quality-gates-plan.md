...
- [ ] 在隔离旧源码与候选源码上完成同一策略测量并验证来源。
- [ ] 补齐真实覆盖缺口及新 workspace 的测量基线。
...
完整静态检查 4/4、Linux 构建 8/8、安全扫描 3/3、E2E/Pi 122/122 均有隔离证据；Chromium 和 WebKit 各 11 项页面回归通过，网络恢复使用受控事件及真实 HTTP/SSE。这里不代表生产、实际 OS 离线恢复或完整托管矩阵通过。基线当前明确标记为工作树测量；提交源码后须再次采集以绑定可复现的提交，随后推送并确认 PR #4 的 `ci/required`。
