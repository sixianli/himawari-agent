...
- 不触碰真实 Mac/Hermes 服务、用户数据库、Keychain 或生产 Secret，不执行生产运行验收、长期观察或部署。
...
## 2026-09-14 独立精确例外审批补充

Owner 在本次 CI 修复任务中明确批准 26 项已核验的测试／文档记录，以及绑定源码和清单摘要的审批入口。范围为 21 条 Gitleaks 当前／历史合成 token 记录、4 组机器扫描误报和 node-forge 1.4.0 的 1 个 README 示例；不更改覆盖率阈值或统计范围。审批由 Codex 按明确授权代发，记录见 [PR #4 审批评论](https://github.com/sixianli/himawari-agent/pull/4#issuecomment-5662807447)，来源提交为 `e2a792f855f82e74519daa9d4bea928370c76c50`，原始清单摘要为 `565c2fda88e8f0852f5dcf1a0b6f27b359e63124e62b7cda7bd194bfdc388c0b`，到期时间为 2026-10-03T00:00:00Z。

- [x] 增加真实 GitHub 评论核验与精确补充清单解析；保留 main 例外、原扫描器、来源和数量验证。
- [x] 对未批准、错误作者、PR／源码／摘要不符、过期、范围扩大、重复和缺失执行回归；验证合入 main 后继承相同批准。相关 5 个测试文件共 153 项通过，初始审批及两个后续边界均保留修复前失败证据。
- [x] 在 Hermes 使用固定工具执行完整机器密钥、Gitleaks、Semgrep 扫描和报告公开检查，结果通过。Semgrep 对新增正则的解析错误已重现，改为明确文本分隔后重新通过完整扫描。
- [x] 使用新构建归档验证依赖文档的公开扫描与安装／进程消费：Linux 构建及公开检查通过，40 个受审阅成员的内容和发现均匹配；用同一归档补跑的 6 项安装／进程测试全部通过。
- [ ] 补齐剩余覆盖率缺口，完成最终全部本地及 GitHub 必需检查。

上述记录是当前未提交候选的局部验证，不代表 PR 或生产部署通过。详细失败与成功证据保存在 `.ci-output/ci-complete-repair/`，包括 `security-review-complete.log`、`security-inheritance-final.json` 和 `current-inputs.json`。


## 2026-09-14 覆盖率采集范围迁移

用户已批准 unit/contracts/tooling/integration 四项目采集、同次 Linux 构建输入及旧基线可比重测。保留增量行 90%、变更函数分支 85% 和已有分组不得回退的要求。

- [x] 扩展采集项目、构建依赖、报告与汇总合同，覆盖缺归档、错身份、缺项目及失败传播。
- [x] 在隔离旧源码与候选源码上完成同一策略测量并验证来源。
- [x] 补齐真实覆盖缺口及新 workspace 的测量基线。
- [ ] 完整本地检查后提交本任务改动，推送 PR #4 并确认托管 ci/required。


## 2026-09-15 增量覆盖率阈值调整

用户针对当前 90%/85% 门槛明确回复“我现在同意把这一个覆盖率改为 80% 和 70%”。该决定替代上一节保留增量阈值的约定；四项目采集、同次构建、可比重测和各 workspace 四类指标不得回退的要求继续有效。

- [x] 将 schema、policy、README、Spec 和执行判定同步为已批准的精确迁移。
- [x] 验证恰好 80%/70% 可以通过，低于门槛、未批准的其他降低或历史基线回退仍被拒绝。Hermes 上对应覆盖率、政策和 SQL 请求检查合计 387 项通过；全量验证另计。
- [ ] 保留旧门槛报告，使用新配置完成正式全量验证及 PR #4 的托管 ci/required。


### 2026-09-15 候选完整验证

Hermes 隔离候选的正式 15 分钟覆盖率作业 `coverage-batched-fixtures` 通过：251 个文件、4,148 项全部通过，零失败、零跳过；变更行 87.387187%，变更函数分支 80.26554%，旧源码按四项目重测后的所有已有 workspace 指标均未回退。采集耗时 818522ms。新增模块基线来自该次完整报告，不采用此前存在失败的诊断报告。

完整静态检查 4/4、Linux 构建 8/8、安全扫描 3/3、E2E/Pi 122/122 均有隔离证据；Chromium 和 WebKit 各 11 项页面回归通过，网络恢复使用受控事件及真实 HTTP/SSE。这里不代表生产、实际 OS 离线恢复或完整托管矩阵通过。该轮基线标记为工作树测量；下述重新采集用于绑定可复现的源码提交，托管状态另行确认。
...
### 2026-09-15 macOS 测试夹具修复

同次托管运行的覆盖率 4148/4148、87.387187%/80.26554% 通过，历史比较无回退；Linux 常规测试、最低 Node、两平台构建与三浏览器通过。macOS 常规测试的 15 项失败集中在三份新增单元测试：模拟 `/proc` 时未固定 Linux 平台、临时目录未解析 macOS 路径别名、服务编排只替换了 Systemd 密钥来源。修正仅涉及测试夹具：保留生产平台拒绝和路径规范化检查，为两种主机密钥来源提供受控替身，显式覆盖两平台的就绪与退出顺序，并补测非 Linux 的命名空间拒绝。Darwin 密钥来源遗漏已在 Linux 模拟测试复现，修正后相关 74 项通过；真实 macOS 的最终结果仍由下一次托管运行确认。
