...
- [ ] 接入目录与网络授权的当前状态、可信父调用关系及 host/runtime/runner/qualification 验证，并在启动点重新核查。
