...
## 2026-09-11 用户实际使用后的修复范围

用户要求解决天气查询约五分钟、模型限流、逐轮活动不可见和审批离开聊天的问题，并已明确批准在 Hermes 开启可撤销的联网搜索授权。查询词可包含对话提供的信息，发送给固定 Exa 服务；这项授权不增加文件、命令或附件权限，凭据检测继续阻止发送。授权保存于 Owner/Agent 的服务端状态，经现有 Gateway 认证、CSRF、修订检查和审计更新；客户端主题偏好不承载执行权限。服务端只将该授权应用于已配置且合格的 web_search、无副作用、无凭据变更、固定接收方的查询。撤销、配置身份变化及范围不匹配后恢复逐次确认。

复用 Pi 0.84.2 的累积消息快照与生命周期事件，对尚未保存的同一消息更新进行合并，并在工具、消息终止和停止边界保存最新快照；不得覆盖已持久化的历史。展示真实活动类型与最近活动时间，不把连接存活当作任务持续推进。已收到原始 thinking 但没有可公开摘要时准确说明；不开放原始推理、签名或 provider 元数据。

审批直接在所属聊天轮次展示，复用 approval.detail / approval.respond 的语义摘要、修订、幂等与近期认证要求。DeepSeek 的当前上游 429 单独诊断，不擅自更换本轮模型。运行环境性能修改必须保持每次校验实际文件字节、路径和权限、拒绝链接及变化检测；不得跨任务缓存安全结论。以同一 Hermes 目录交替测量与真实网页流程验证性能。
...
Owner 于本轮批准 [ADR 0028](../../adr/0028-protected-runtime-installation.md)。受保护安装记录由 root 持有，运行账号不能修改程序、父目录、启动入口或可信版本记录；首次完整审计后，只在该保护和版本身份仍成立时复用结果。动态业务授权和工具隔离继续逐次执行。未配置此机制的现有部署保持全量核验；失败明确拒绝。当前源码有实现和受控回归，Hermes 管理员操作与实机拒绝写入验收尚未执行，不能宣称正式提速。
