...
- 三语 UI 控制；模型回答语言由对话要求决定，不设置独立选择器。
...
- 按 2026-09-12 所有者确认的范围，只保留 UI locale 控件；切换 UI 不修改 Thread 或向模型注入回答语言要求。
- 模型依据用户提问和对话要求选择回答、摘要与解释的语言；原始代码、日志、引用、命令和专有名词保持原文。
...
不提供回答语言控件。UI locale 菜单只保存当前浏览器的界面偏好，不调用 Thread 设置接口，不控制模型回答语言。用户在对话中提出的语言要求作为当前任务内容处理。
