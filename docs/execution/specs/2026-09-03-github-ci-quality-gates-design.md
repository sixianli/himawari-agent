...
- 代替上线验收实现发布状态机、运行与故障恢复、平台资格、签名和升级操作。
...
| `coverage` | Ubuntu 基线 Node | `policy`、同次 Linux `build` | 受控的 unit/contracts/tooling/integration 覆盖率、增量与可比基线策略通过 |
...
使用与 Vitest `4.1.9` 精确匹配的 coverage provider。覆盖率 job 对 unit/contracts/tooling/integration 的进程内测试计算门禁，显式 include 全部 `apps/*/src`、`packages/*/src` 的生产 TS/TSX 和 `scripts/ci` 的自有执行代码。排除仅限声明文件、明确生成物、测试夹具和原样引入的第三方工具；`packages/testing` 按测试夹具责任显式登记。独立进程 integration、浏览器与 Pi 上游包另行验证，不假装已合并它们的执行覆盖率。

2026-09-14 用户批准扩大采集项目。coverage 必须核验并消费同次 Linux build 的归档和 context；缺失或身份不符时不得开始采集，最终汇总核对其消费摘要。报告必须包含四个项目的完整执行，缺项、失败或跳过均阻断。旧政策仅迁移这一固定的新增项目、构建依赖和归档报告要求，其他已接受合同保留；增量阈值按下述 2026-09-15 授权调整。旧基线与候选使用同一新策略测量，不能用未更新的三项目数字直接比较。

2026-09-15 用户明确批准将新增或修改的可执行行覆盖率门槛改为 80%，含有变更的函数中可定位分支覆盖率门槛改为 70%。分母为 source-map 还原后的可执行位置，不能用文本行数、注释或未定位分支替代。无新增可执行行或无相关分支时记录精确原因和 diff 摘要，完整覆盖率报告仍须存在。

首次启用前实测并提交各生产 workspace 及自有 CI 工具的固定基线，之后 lines、branches、functions、statements 均不得低于基线。现有不足通过明确的初始基线表达，不把增量门槛误写为全仓覆盖率。已有未覆盖代码本次被修改时适用增量规则。未来提高基线由验证后的显式变更完成，CI 不自动回写。
...
基线和例外默认读取目标分支受保护版本；同一 PR 不能自行降低阈值使自身通过。2026-09-15 Owner 已明确批准本次 90%/85% → 80%/70% 的精确迁移，CI 仅对这一旧值与新值组合应用新阈值；其他阈值提议仍按目标分支值判断，低于 80%/70% 的配置直接拒绝。该授权不改变各 workspace 四类历史基线。覆盖率工具版本、include 范围或映射算法改变时，先用相同策略在隔离目录对旧基线和候选做可比测量。无法建立比较时阻断政策迁移，不以旧数字硬判通过。

授权、撤销、预算、Secret/Payload、路径边界、幂等和崩溃恢复还必须有对应的负向行为测试。80%/70% 是代码执行证据阈值，不是安全正确性的替代。

### 7. 安全与例外政策

密钥检测保留现有 `scan-machine-secrets.mjs`，增加固定版本 Gitleaks。PR 扫描候选 head 的完整祖先历史和当前受跟踪内容，包含目标分支中已接受例外的来源提交，检测曾提交后删除的泄漏；不扫描候选祖先之外的无关分支。默认分支和周期检查执行全部可达历史扫描。例外仅限经过审阅的合成测试样例，真实凭据泄漏不可豁免。
...
2026-09-14 经 Owner 明确批准，新增独立的精确清单审批入口，用于尚未进入目标分支的新测试或依赖文档值。常规例外仍从目标分支读取，候选分支不能自行替换它们。`.github/security-review-manifest.json` 保存审核过的原始清单，`.github/security-review-comment.json` 只定位 PR 评论；本地文件中的批准声明不是授权证据。检查通过固定 GitHub HTTPS API 读取评论，核验仓库所有者的数字身份与登录名、仓库和 PR、被审核源码提交、清单原始字节摘要、批准决定与最长 30 天的有效期。新增或修改清单的 PR 还须匹配本次事件编号和 head；来源提交必须属于被测提交的历史。网络失败、缺少记录、错误作者、过期或范围变化均失败。

该入口只能补充批准清单列出的合成 token、机器扫描误报和依赖文档示例。token 继续核验不可变源码、字面量分类、历史范围及数量；原有条目使用目标分支的来源证据，新增条目只使用批准提交的对应证据。机器扫描器仍取目标分支版本，报告保留所有发现；批准仅匹配精确路径、规则、摘要及数量，新增、重复或消失的匹配均拒绝。依赖文档使用下述既有成员校验，不扩大为包或目录豁免。清单及评论引用已经原样合入目标分支后，后续 PR 继承该分支接受的精确范围，继续核验评论和有效期；修改清单仍需新的明确批准。既有例外不因这条补充路径延长有效期。

构建归档包含锁定 npm 包原样发布的文档、测试和源映射。对其中已核实为占位值、未求值插值、认证说明或合成密码学测试材料的命中，使用 `ci/security-exceptions.json` 中或上述独立批准清单转换得到的 `published-synthetic-fixture` 精确记录；公开来源本身不构成豁免理由。记录必须绑定包名、版本、锁文件路径、官方发布包完整性摘要、归档成员路径、成员文件摘要，以及每条扫描规则、字面量摘要和准确次数，并保留用途、来源核对依据、Owner 和到期日。任意成员内容、依赖身份或命中集合变化均须重新审阅，不允许按包或目录放行。公开输出检查读取目标分支已接受的例外，并按上述独立审批合同补充精确批准条目；仅在初始化时目标分支的政策和例外均不存在，才按明确的初始化提案路径验证候选政策，并保留待 Owner 审阅状态。缺少执行上下文时不应用例外；敏感哨兵和真实凭据始终阻断。
...
Mac/Hermes 平台 conformance、六类正式浏览器、人工 WCAG、运行与故障恢复和核心升级恢复由生产验收流程执行。按 2026-09-04 Owner 决定，长期连续运行观察不设上线前置时长。发布证据须在 Actions retention 到期前转入经批准的持久证据位置并重新核验；默认不上传数据库、Secret、用户正文或未脱敏 Trace。
