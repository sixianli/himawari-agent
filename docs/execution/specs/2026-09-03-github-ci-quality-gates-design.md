...
依赖检查包含生产与开发依赖及可枚举的传递依赖。使用锁文件和官方 advisory 数据，记录扫描时间与数据库/响应摘要。未经批准的 High/Critical 发现阻断；Low/Moderate 仍输出报告。查询网络错误、限流或不完整响应属于 `infrastructure_failed`，不是零漏洞。

advisory 请求最多执行两次，每次保持 60 秒期限，重试间隔最多 1 秒，总预算约 121 秒，job 的 15 分钟上限不变。仅当尚未收到响应且本次自有期限触发 `TimeoutError` 时重试一次；HTTP 错误、响应体失败、无效内容、其他网络错误及漏洞发现不重试。每次请求的身份、耗时和结果均写入 `requestAttempts`，成功重试也保留首次失败，`CheckResult.retryCount` 记录实际重试次数。两次请求使用相同 endpoint 和请求内容，最终只接受完整校验后的响应；耗尽后仍阻断。
...
| 下载、advisory API、runner 或 artifact 服务不可用 | `infrastructure_failed`；有界重试保留 attempt，耗尽后仍阻断 |
