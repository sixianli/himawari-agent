...
`sandbox-scope.v1` 定义受保护 scope 正文：Owner/Agent/Thread/Run/toolCallId 与父调用引用、host、Handle/input、操作、授权与模型、profile、目录授权引用/版本/根身份、可空网络授权引用及有效期。正文不包含原始 SRT 配置或机器秘密。`SandboxScopeService` 复用现有 PayloadStore/Protector，限制密文与正文大小，按原始字节验证 SHA-256，并与当前候选计划逐项核对；准入前失败不消费权限。父调用引用与目录/网络授权引用的存在不表示其当前有效性已被证明，仍需可信存储和目标主机验证。scope 不能签发主机资格或替代文件使用时检查。候选计划使用不含凭证摘要的严格 schema，与最终计划共用字段及期限校验，不伪造尚未消费的凭证。
