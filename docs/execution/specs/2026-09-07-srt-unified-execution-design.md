...
R6 的正式薄适配复用 `RuntimeToolPort` 与 Pi 扩展定义注册：模型名称为 `execution_task_start/status/output/cancel`，产品 action 为 `execution.task.start/status/output/cancel`。start 选择现有已授权 Handle 和冻结输入，其中保存已批准的 command/cwd/timeout；不会根据模型参数新建权限或放宽前台 Bash。管理调用按不透明 resourceRef 查原 Run 的持久关联，核对原调用回执、当前 Grant、模型、线程和 fence。调用身份及查询结果仍保存到现有 Run artifact。

普通后台 Bash 复用 R5 的 Pi Bash 工厂和同一受限 Operations；只有 Worker 传入的 executionMode 可改变输出形态，前台结果保持原格式。后台 Bash 在完整行检查后输出，末尾未换行内容在退出时检查，秘密命中即停止并拒绝该段。

Job Host 的实际进程启动确认与准备完成、命令结束分别返回。后台 start 的 `started` 回执不可改写，Worker 返回它后继续监督、复核授权并追加输出；启动确认丢失只保留未知结果和原关联。输出为按到达顺序合并的 stdout/stderr，每片最多 32 KiB、总量受原 maxOutputBytes 约束；片段和结束标记不可改写，游标只引用受保护片段位置。分页返回 base64，完整可显示文本另附 text，避免 UTF-8 跨页字节损失；结束片段持久保存命令退出码、结束原因和主进程退出事实，status 可查询它，不能覆盖初始 started 回执；输出对模型的披露仍要求原调用的有效授权和分类上限。

本批受控测试服务使用安装声明 `readinessProbes`：`unix_http`、唯一 ref、私有 socket 文件名、HTTP path、预期 2xx 状态及最长 30 秒期限。Worker 从声明解析 `readinessProbeRef`，准备和启动前核对；SRT 只额外允许任务私有目录内声明的一个 Unix socket，保持禁止通用本地 TCP bind、其他 Unix socket 和未授权域名。固定的基础设施探针在同一 SRT 策略下请求该 socket，只检查 HTTP 状态，丢弃响应正文，限制响应头和总时间。探针有单独的原进程取消与退出观察，不从任务日志提取 ready；受保护 Job Host 证据保存探针摘要和就绪时间，首次就绪证据在后续资源观察中保留。探针本身是有界基础设施开销，不计入用户进程树的采样指标；该指标同样不含 Job Host。

Run 正常结束前停止原 Run 的后台资源；仍有未知清理义务时转入已有 `reconciling_external_result`。SQLite 完成事务再次检查未释放资源，避免枚举与完成之间的新准入竞态。取消和执行中断也触发原资源停止；Worker 持续复核时遇到撤权、Run 结束或 Agent 通道丢失则停止原 Job Host。Mac 主进程退出不自动释放目录占用；保留 unknown/隔离，等待既有核查机制。
...
通用 scope 沿已有审批快照的目录、主机、确切域名和模型披露对象取得范围；版本/模式、目录授权和父调用仍逐次复核。资源 output 请求只返回已有输出快照的受保护分页引用，不在该请求中重新执行命令。cursor 绑定原调用、资源、语义及输出摘要，并以既有 Run trace 持久保存；未签发或跨绑定 cursor 拒绝。页的 truncated 表示分页传输截断，原工具输出的来源与截断信息仍须随受保护正文保留。空快照与无输出可观察地区分；后台运行中的输出由 R6 通过连续受保护片段采集，分页和终止信息按第 4.2 节处理。
