...
作业观察校验只约束现有持久账本将要保存的事实，不建立新 Agent 状态机。它拒绝身份/策略更换、序号重放、完成后再次启动及未知副作用自动重放；只有确认清理且副作用可核实的结果才能进入 completed/failed。隔离槽位可以进入 reconciling 后继续核查，不能直接恢复执行。重复读取已有同一回执是读操作，不应伪装成新 sequence。恢复、取消、费用与模型调用次数仍归 Run/Handle/现有模型账本管理；资源天花板由当前 Capability 回执投影。SQLite 作业账本已通过追加迁移实现序号 CAS、启动前权限同事务复核、观察重放及待核查分页；终态输出必须对应已持久化的 invocation 结果。它不签发主机资格，也不替代真实清理证明。`SandboxJobJournalPort.admit()` 将既有凭证消费与首条作业记录放入同一 SQLite 事务，凭证摘要由消费结果生成；建账失败回滚 Handle 消费。独立的准备写入口不可调用。旧/重放凭证没有作业记录时属于执行未知，直接拒绝准入，不得补建并启动。正式监督器必须使用这一原子入口，并完成 scope 与资格检查。既有 `WorkerDelegationAdmissionService` 支持可信组合显式选择 SRT 准入模式：先准备 scope/资格绑定，再调用原子账本，并使用同事务返回的冻结凭证构造首次 Worker 消息；失败不回退到独立消费路径，重放不派发。生产组合尚未提供真实 scope/资格解析，执行消息的可选 `sandboxJob` 身份只由可信准入填入；消息合同核对 invocation/Owner/Agent/Run，认证 UDS 核对 receiptRef。旧消息保持原格式。当前 Worker 在 v2 转换旧执行请求之前，以 `SANDBOX_SUPERVISOR_UNAVAILABLE` 拒绝 SRT 作业，避免身份丢失后误用旧后端。Job Host 组件与 `SandboxJobLifecycleService` 协调器已实现；协调器复用同一账本进行启动 CAS、取消和未知隔离，Worker 产品适配器保留输出保护与原期限。正式跨进程账本、当前主机权限/资格解析及生产恢复接线仍待实现。

`sandbox-scope.v1` 定义受保护 scope 正文：Owner/Agent/Thread/Run/toolCallId 与父调用引用、host、Handle/input、操作、授权与模型、profile、目录授权引用/版本/根身份、可空网络授权引用及有效期。正文不包含原始 SRT 配置或机器秘密。`SandboxScopeService` 复用现有 PayloadStore/Protector，限制密文与正文大小，按原始字节验证 SHA-256，并与当前候选计划逐项核对；准入前失败不消费权限。目录 scope 同时记录目录授权引用、版本、根身份、授权来源与所需文件操作集合。读取器通过当前 `HostFileStatePort.readGrant()` 复核主机、版本、根、授权、撤销/有效期、路径/挂载策略和操作范围，不能把读取许可扩大为写入或删除许可。`parentRequestId` 必须与准入请求 causationId 一致，正式处理器继续校验该父请求；父工具调用关系仍需工作流登记核实。正式目录状态来源、网络授权与目标主机验证尚未接通。scope 不能签发主机资格或替代文件使用时检查。候选计划使用不含凭证摘要的严格 schema，与最终计划共用字段及期限校验，不伪造尚未消费的凭证。
