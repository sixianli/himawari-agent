...
contract_sha256: "sha256:6c6782ca49bf8e80a48ff50222407c7afeaa0daa85be76590c8e036271e70a71"
supersedes: ""
superseded_by: ""
date: "2026-08-27"
---
...
安装产物包含 Agent Service、Execution Worker、admin CLI 及产品运行时包；它不包含 `packages/testing` 的生产 adapter。每个构建固定提交内容、package-lock、workspace checksum、Node 平台/架构和外部依赖闭包。由于 `better-sqlite3` 等 native 依赖，Mac 与 Linux 必须分别构建和验收，不能把一个平台的二进制包当作另一个平台的 immutable artifact。
...
7. 运行只读 doctor、db status 和适用业务查询；确认 Agent Service 通过 UDS handshake、没有 testing adapter、没有 repository checkout 路径，也没有秘密或私人正文输出。
