...
覆盖率只合并 unit/contracts/tooling 的进程内证据，完整包含未导入生产文件与自有 CI 工具。新代码的可执行行和可定位分支分别受 90%/85% 阈值约束，workspace 四类指标由接受的目标分支基线控制。安全扫描组合原有机器密钥检查、Gitleaks 当前内容与提交历史、固定 Semgrep CE 规则和完整依赖 advisory；例外须有精确范围、来源与 UTC 到期日，首次合成夹具提案与已接受例外分别标识。 advisory 的单次请求和响应体读取失败分别保留请求摘要、阶段、耗时、超时信号及最多三层已知网络错误身份。任意错误消息、堆栈和请求内容不进入公开诊断；60 秒请求预算和失败语义保持不变。
