...
生产 `RuntimeToolPort.listAuthorized()` 也可返回尚未授权的文件读取请求 `request_file_read`，其 `capabilityHandleRef` 明确为 `null`，不需要伪造凭证。模型仅提出 `hostRef/path/maximumBytes` 意图；产品再次验证参数。当前授权签发链路未接通，preflight 返回 `FILE_READ_AUTHORIZATION_UNAVAILABLE`，不会派发 Worker。原有字符串 Handle 工具仍仅允许选择已签发的 `inputRef`。文件内容限定为单个 UTF-8 普通文本、最多 64 KiB；路径解析与实际文件检查仍待后续实施。[SOURCE: docs/execution/specs/2026-09-07-real-file-summary-agent-loop-design.md]
