CREATE TABLE model_invocation_identities (
  ...
  source TEXT NOT NULL CHECK (source IN ('model-port', 'agent-stream')),
...
CREATE INDEX model_invocation_identities_scope
  ON model_invocation_identities(owner_id, agent_id, run_id, logical_slot, sequence);

CREATE INDEX model_invocation_identities_status
  ON model_invocation_identities(owner_id, agent_id, status);
